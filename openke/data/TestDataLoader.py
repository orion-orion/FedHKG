...
class TestDataLoader(object):
	...
	def __init__(self, in_path = "./", sampling_mode = 'link', type_constrain = True):
		...
		"""for link prediction"""
		self.lib.getHeadBatchTest.argtypes = [
			ctypes.c_void_p,
			ctypes.c_void_p,
			ctypes.c_void_p,
		]
		self.lib.getTailBatchTest.argtypes = [
			ctypes.c_void_p,
			ctypes.c_void_p,
			ctypes.c_void_p,
		]
	...
	def sampling_lp(self):
		res = []
		self.lib.getHeadBatchTest(self.test_h_addr, self.test_t_addr, self.test_r_addr, "test")
		res.append({
			"batch_h": self.test_h.copy(), # predict head, its neg sample
			"batch_t": self.test_t[:1].copy(), 
			"batch_r": self.test_r[:1].copy(),
			"mode": "head_batch"
		})
		self.lib.getTailBatchTest(self.test_h_addr, self.test_t_addr, self.test_r_addr)
		res.append({
			"batch_h": self.test_h[:1], 
			"batch_t": self.test_t, # predict tail，its neg sample
