...
class ValidDataLoader(object):
	...
	def __init__(self, in_path = "./", sampling_mode = 'link', type_constrain = True):
		...
		"""for link prediction"""
		self.lib.getHeadBatchValid.argtypes = [
			ctypes.c_void_p,
			ctypes.c_void_p,
			ctypes.c_void_p,
		]
		self.lib.getTailBatchValid.argtypes = [
			ctypes.c_void_p,
			ctypes.c_void_p,
			ctypes.c_void_p,
		]
	...
	def sampling_lp(self):
		res = []
		self.lib.getHeadBatchValid(self.valid_h_addr, self.valid_t_addr, self.valid_r_addr)
		res.append({
			"batch_h": self.valid_h.copy(), # predict head, its neg sample
			"batch_t": self.valid_t[:1].copy(), 
			"batch_r": self.valid_r[:1].copy(),
			"mode": "head_batch"
		})
		self.lib.getTailBatchValid(self.valid_h_addr, self.valid_t_addr, self.valid_r_addr)
		res.append({
			"batch_h": self.valid_h[:1], 
			"batch_t": self.valid_t, # predict tail，its neg sample
